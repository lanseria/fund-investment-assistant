...
export const useHoldingStore = defineStore('holding', () => {
   ...
   * 更新持仓金额
   */
  async function updateHolding(code: string, data: { shares?: number | null, costPrice?: number | null, attentionLevel?: number | null, operationStrategy?: string | null }) {
    try {
  ...
  /**
   * 刷新基金的实时估值 (后端执行)
   * scope: 'user' (仅我的基金) | 'all' (全站，需管理员)
   */
  async function refreshServerEstimates(scope: 'user' | 'all' = 'user') {
    ...
    try {
      ...
      alert(`刷新失败: ${error.data?.statusMessage || error.message}`)
    }
    finally {
