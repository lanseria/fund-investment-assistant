...
const TASK_GROUPS: Record<'dashboard' | 'dailyOps', { intro: string, tasks: ScheduledTask[] }> = {
  ...
  dashboard: {
    ...
    tasks: [
      ...
      { time: '09:00', name: 'fund:processTransactions', desc: '交易结算：将「待处理」交易按最新确认净值成交（先卖后买），更新持仓与现金；基金转换先确认转出，再按实际到账金额买入。' },
  ...
  dailyOps: {
    intro: '交易从生成到结算的生命周期由以下定时任务自动推进（北京时间）：AI 于交易日 14:30 生成决策，「待处理」交易在每日 09:00 按最新确认净值正式成交。',
    tasks: [
